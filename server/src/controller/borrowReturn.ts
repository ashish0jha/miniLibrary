...
export async function returnBookHandler(req: Request, res: Response) {
  ...
  try {
    ...
    let {id,available_copies,title:bookName,total_copies} = result.rows[0];

    const checkQuery = `SELECT * FROM borrow_records WHERE user_id = $1 AND book_id = $2 AND returned_date IS NULL` 

    const checkResult = await client.query(checkQuery,[user_id,id]);
    ...
    if(available_copies > total_copies) {
        throw new Error("Available copy can't exceed total_copy")
    }

    const incrementQuery = `UPDATE books SET available_copies = $1 WHERE id = $2`;
    await client.query(incrementQuery,[available_copies,id]);

    const returnQuery = `UPDATE borrow_records SET returned_date = $1 WHERE user_id = $2 AND book_id = $3 AND returned_date IS NULL`;

    const date = new Date();
    await client.query(returnQuery,[date,user_id,id]);
    ...
    res.json({msg:"Return Successful",data:toReturn})
