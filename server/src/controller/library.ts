...
export async function leaderboardHandler(req: Request, res: Response) {
  try {
    const query = `SELECT DENSE_RANK() OVER(ORDER BY COUNT(bb.book_id) DESC) AS rank, b.id AS book_id, b.title AS book_title,b.author AS book_author,COUNT(bb.book_id) AS borrow_count FROM borrow_records bb RIGHT JOIN books b ON bb.book_id = b.id GROUP BY b.id, b.title,b.author ORDER BY borrow_count DESC`;
...
export async function totalBorrowesBystudentHandler(req:Request,res:Response) {
    try{
        const query = `SELECT b.title AS book_title,b.author AS book_author,bb.borrowed_at AS borrow_date FROM borrow_records bb JOIN books b ON bb.book_id = b.id WHERE bb.user_id = $1 AND returned_date IS NULL`;
